#!/usr/bin/env tsx
/**
 * Generate LICENSE file for each publishable package.
 * - OSS: Apache-2.0 standard text
 * - Pro: BUSL-1.1 with proper params (Change Date = +4 years from publish)
 */
...
const APACHE_2_0_TEXT = `                                 Apache License
   ...
   Copyright 2026 Milo Nguyen, Orqenix Contributors
...
function generateBUSL11(changeDate: string): string {
...
async function main(): Promise<void> {
  ...
  for (const pkg of packages) {
    const licensePath = join("packages", pkg, "LICENSE");
    try {
